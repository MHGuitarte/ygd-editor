...
T = {
 ...
 'cli_h': 'Bez ostrzeżenia: zainstaluj z Terminala.',
 'cli_p': 'Wklej to do Terminala (Aplikacje › Narzędzia). Pobiera najnowszą wersję dla twojego Maca, sprawdza jej sumę kontrolną i to, że jest podpisana certyfikatem projektu, i umieszcza ją w folderze Aplikacje. macOS o nic nie pyta, bo nic nie przeszło przez przeglądarkę. Uruchom ponownie w dowolnej chwili, żeby zainstalować od nowa.',
 'cli_copy': 'Kopiuj', 'cli_copied': 'Skopiowano',
 'cli_or': 'Albo z obrazem dysku z przycisku powyżej:',
 ...
 'g_sections': [
  ...
  ('current', 'Testowanie worktree w domyślnym', [
    'Miejscem do wypróbowania zmiany jest folder samego repozytorium — <em>domyślny worktree</em> — bo tam są już <code>node_modules</code>, <code>.env</code>, pamięć podręczna budowania i działający serwer deweloperski. Praca odbywa się w pozostałych worktree. <strong>Ustaw jako bieżący</strong> wnosi jeden z nich do domyślnego worktree: jego commity, to, czego jeszcze nie zacommitował, i jego nowe pliki, na gałęzi domyślnego worktree. Od tej chwili nagłówek na każdym ekranie mówi, co uruchamia domyślny worktree — <code>DOMYŚLNY · feat-a + feat-b · 1 edycja</code> — a pasek boczny oznacza worktree <em>domyślny</em> i <em>bieżące</em>.',
    '<img src="{img}current.png" alt="Pasek w nagłówku, rozwinięty: skąd przyszedł domyślny worktree, dwa bieżące worktree i co można zrobić dalej" loading="lazy">',
    '<strong>Zamień</strong> wstawia na jego miejsce inny worktree. <strong>Dodaj</strong> nakłada kolejny, żeby wypróbować dwie zmiany razem; gdzie oba dotykają tych samych linii, wczytywanie zatrzymuje się na konflikcie w domyślnym worktree, a dla każdego pliku zostawiasz stronę domyślną, bierzesz przychodzącą albo edytujesz go i oznaczasz jako rozwiązany — potem <em>Kontynuuj</em> albo <em>Przerwij</em>, by wrócić do tego, co działało wcześniej.',
    '<strong>Poprawki zrobione podczas testów wracają do domu.</strong> Jeśli ty — albo agent w domyślnym worktree — poprawisz coś podczas próby, <em>Odeślij edycje</em> zwraca to do worktree, do którego należy, jako niezacommitowane zmiany do przejrzenia i zacommitowania tam; <em>Cofnij</em> odwraca to aż do następnego kroku. O plik należący do kilku bieżących worktree albo do żadnego aplikacja pyta, zamiast zgadywać. Zamień, Dodaj, Odśwież i Zakończ najpierw odsyłają edycje, więc nic zrobionego w domyślnym worktree nie przepada.',
    '<strong>Zakończ</strong> oddaje domyślny worktree dokładnie w takim stanie, w jakim był: jego gałąź i jego własne zmiany, dodane do indeksu i nie. Każdy krok najpierw otwiera krótki podgląd: co wchodzi, co czeka z boku, które pliki weszłyby w konflikt i co blokuje go teraz, razem z wyjściem. Znajdziesz je w menu <code>···</code> worktree, pod prawym przyciskiem w pasku bocznym, w pasku nagłówka i pod ⌘⇧D / ⌘⇧E.',
    'Ustawienia › Git mogą same odświeżać bieżący worktree, gdy jego agent dalej pracuje, i uruchamiać polecenie po każdym wczytaniu — na przykład <code>pnpm install</code>, gdy zmienił się <code>pnpm-lock.yaml</code> — w terminalu domyślnego worktree, gdzie je widzisz. Twój <code>git stash</code> nigdy nie jest ruszany. Wymaga git 2.40 lub nowszego.',
  ]),
  ...
  ('trouble', 'Gdy coś pójdzie nie tak', [
    ...
    '<p><strong>Jak w ogóle uniknąć ostrzeżenia.</strong> Zainstaluj z Terminala: <code>curl -fsSL https://mhguitarte.github.io/ygd-editor/install.sh | bash</code>. Sprawdza pobrany plik tak, jak zrobiłby to macOS, i umieszcza aplikację w folderze Aplikacje; więcej na stronie pobierania.</p>'
  ...
  ('uninstall', 'Odinstalowanie', [
    '<strong>Zanim ją usuniesz</strong> — oba kroki są opcjonalne. Jeśli w domyślnym worktree jest <em>bieżący</em> worktree, kliknij <em>Zakończ</em>, żeby domyślny worktree odzyskał swoją gałąź i zmiany. Aby wylogować się z dostawcy, użyj <em>Odłącz</em> na jego karcie w Ustawienia › Dostawcy AI: logowania należą do Claude Code, Codex i Gemini CLI, a nie do aplikacji, więc usunięcie aplikacji ich nie kończy. Potem zamknij ygd-editor.',
    '<strong>macOS.</strong> Przeciągnij <strong>ygd-editor</strong> z folderu Aplikacje do Kosza — z <code>~/Applications</code>, jeśli umieściła ją tam instalacja z Terminala.',
    '<strong>Windows.</strong> Ustawienia › Aplikacje › Zainstalowane aplikacje › ygd-editor › <em>Odinstaluj</em> albo Panel sterowania › Programy i funkcje.',
    '<strong>Linux.</strong> Usuń plik AppImage. Dla pakietu Debian / Ubuntu: <code>sudo apt remove ygd-editor</code>.',
    '<strong>Co zostaje.</strong> Twoje ustawienia, przestrzenie robocze, historia sesji i pobrane Codex i Gemini CLI są w folderze <code>.ygd-editor</code> w twoim katalogu domowym, a odinstalowanie ich nie usuwa, więc po ponownej instalacji wszystko wraca. Aby usunąć także je, razem z pamięcią podręczną aplikacji, uruchom to po odinstalowaniu:',
    ...
    '<strong>Co pozostaje nietknięte.</strong> Twoje repozytoria i utworzone worktree to zwykłe foldery git i zostają na swoim miejscu; <code>git worktree remove &lt;folder&gt;</code> usuwa jeden z nich. Jeśli aplikacja zainstalowała za ciebie Claude Code, zostaje on w twoim katalogu domowym — <a href="https://docs.anthropic.com/en/docs/claude-code">dokumentacja Anthropic</a> wyjaśnia, jak go usunąć.',
  ]),
