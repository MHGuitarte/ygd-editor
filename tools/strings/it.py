...
T = {
 ...
 'cli_h': 'Senza l’avviso: installala dal Terminale.',
 'cli_p': 'Incolla questo nel Terminale (Applicazioni › Utility). Scarica l’ultima versione per il tuo Mac, ne controlla il checksum e che sia firmata con il certificato del progetto, e la mette in Applicazioni. macOS non chiede nulla, perché niente è passato da un browser. Eseguilo di nuovo quando vuoi per reinstallare.',
 'cli_copy': 'Copia', 'cli_copied': 'Copiato',
 'cli_or': 'Oppure, con l’immagine disco del pulsante qui sopra:',
 'need_h': 'Cosa ti serve', 'need_p': 'Git sul tuo computer e un account presso almeno un provider. <a href="https://github.com/openai/codex">Codex</a> e <a href="https://github.com/google-gemini/gemini-cli">Gemini CLI</a> non sono nell’installer — è questo che lo tiene piccolo —: l’app scarica quello che scegli la prima volta che lo chiedi (circa 100 MB per Codex, 20 MB per Gemini CLI) e lo verifica contro la versione con cui è stata costruita questa release prima di eseguirlo; per <a href="https://docs.anthropic.com/en/docs/claude-code">Claude Code</a>, la cui licenza non ne consente la distribuzione, Impostazioni › Provider di IA offre un clic che esegue per te l’installer ufficiale di Anthropic. Collegare un provider apre la sua pagina di accesso nel browser. Per le pull request, <code>gh</code> di GitHub o <code>glab</code> di GitLab.',
 ...
 'g_sections': [
  ...
  ('current', 'Provare un worktree in quello predefinito', [
    'Il posto per provare una modifica è la cartella del repository stesso — il <em>worktree predefinito</em> — perché lì ci sono già <code>node_modules</code>, <code>.env</code>, la cache di build e il server di sviluppo. Il lavoro si fa negli altri worktree. <strong>Rendi corrente</strong> ne porta uno nel worktree predefinito: i suoi commit, ciò che non ha ancora committato e i suoi file nuovi, sopra il branch del worktree predefinito. Da allora l\'intestazione dice, in ogni schermata, cosa esegue il worktree predefinito — <code>PREDEFINITO · feat-a + feat-b · 1 modifica</code> — e la barra laterale contrassegna il worktree <em>predefinito</em> e quelli <em>correnti</em>.',
    '<img src="{img}current.png" alt="La striscia dell\'intestazione, aperta: da dove veniva il worktree predefinito, i due worktree correnti e cosa si può fare dopo" loading="lazy">',
    '<strong>Scambia</strong> mette un altro worktree al suo posto. <strong>Aggiungi</strong> ne impila un altro sopra, per provare due modifiche insieme; dove entrambi toccano le stesse righe il caricamento si ferma su un conflitto nel worktree predefinito, e per ogni file tieni il lato predefinito, prendi quello in arrivo o lo modifichi e lo segni risolto — poi <em>Continua</em>, oppure <em>Annulla</em> per tornare a ciò che girava prima.',
    '<strong>Le correzioni fatte durante i test tornano a casa.</strong> Se tu — o un agente nel worktree predefinito — correggi qualcosa mentre provi, <em>Rimanda le modifiche</em> lo restituisce al worktree a cui appartiene, come modifiche non committate da rivedere e committare lì; <em>Annulla</em> lo riporta indietro fino al passo successivo. Un file che appartiene a più worktree correnti, o a nessuno, viene chiesto invece che indovinato. Scambia, Aggiungi, Aggiorna e Termina rimandano prima le modifiche, così nulla di ciò che è stato fatto nel worktree predefinito va perso.',
    '<strong>Termina</strong> restituisce il worktree predefinito esattamente com\'era: il suo branch e le sue modifiche, in stage e non. Ogni passo apre prima una breve anteprima: cosa entra, cosa viene messo da parte, quali file andrebbero in conflitto e cosa lo impedisce adesso, con la via d\'uscita. Li trovi nel menu <code>···</code> di un worktree, con il tasto destro nella barra laterale, nella striscia dell\'intestazione e con ⌘⇧D / ⌘⇧E.',
    'Impostazioni › Git può aggiornare da solo un worktree corrente quando il suo agente continua a lavorare, ed eseguire un comando dopo ogni caricamento — <code>pnpm install</code> quando è cambiato <code>pnpm-lock.yaml</code>, per esempio — nel terminale del worktree predefinito, dove lo vedi. Il tuo <code>git stash</code> non viene mai toccato. Serve git 2.40 o successivo.',
  ]),
  ('providers', 'Agenti e provider', [
    ...
    '<p>Impostazioni › Provider di IA collega ogni provider con il tuo account: <em>Collega</em> apre la pagina di accesso del provider nel browser, e il riquadro mostra poi l’account, il piano e <em>Cambia account</em> / <em>Scollega</em> / <em>Prova la connessione</em>. Codex e Gemini CLI li scarica l’app la prima volta che li scegli — la scheda indica versione e dimensione, mostra l’avanzamento e <em>Rimuovi download</em> lo toglie di nuovo —; se hai una tua copia installata, l’app usa quella. La licenza di Claude Code non ne consente la distribuzione, quindi il suo riquadro offre <em>Installa Claude Code</em>, che esegue l’installer ufficiale di Anthropic nel terminale del riquadro, nella tua cartella personale, senza password di amministratore. L’agente e il modello predefiniti, e la preimpostazione dei permessi, possono cambiare da repository a repository.</p>',
  ]),
  ...
  ('trouble', 'Quando qualcosa va storto', [
    ...
    '<p><strong>Evitare del tutto l’avviso.</strong> Installala dal Terminale: <code>curl -fsSL https://mhguitarte.github.io/ygd-editor/install.sh | bash</code>. Controlla il download come avrebbe fatto macOS e mette l’app in Applicazioni; vedi la pagina di download.</p>'
    ...
    '<p><strong>Un provider risulta «Non installato».</strong> Codex e Gemini CLI si scaricano su richiesta: premi <em>Scarica</em> sulla scheda; se il download fallisce, la scheda dice perché (offline, oppure un file che non corrispondeva al suo checksum ed è stato scartato). Per Claude Code, fai clic su <em>Installa Claude Code</em> nel suo riquadro; se lo hai installato tu, apri un terminale, verifica che <code>claude --version</code> funzioni, poi Impostazioni › Provider di IA › Ripeti la ricerca.</p>'
