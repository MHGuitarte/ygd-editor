...
T = {
 ...
 'cli_h': 'Uyarı olmadan: Terminal’den kur.',
 'cli_p': 'Bunu Terminal’e yapıştır (Uygulamalar › İzlenceler). Mac’in için en son sürümü indirir, sağlama toplamını ve projenin sertifikasıyla imzalandığını denetler, sonra Uygulamalar klasörüne koyar. macOS hiçbir şey sormaz, çünkü hiçbir şey tarayıcıdan gelmedi. Yeniden kurmak için istediğin zaman tekrar çalıştır.',
 'cli_copy': 'Kopyala', 'cli_copied': 'Kopyalandı',
 'cli_or': 'Ya da yukarıdaki düğmeden disk görüntüsüyle:',
 ...
 'g_sections': [
  ...
  ('current', 'Bir worktree\'yi varsayılan worktree\'de test etmek', [
    'Bir değişikliği denemenin yeri deponun kendi klasörüdür — <em>varsayılan worktree</em> — çünkü <code>node_modules</code>, <code>.env</code>, derleme önbelleği ve çalışan geliştirme sunucusu zaten oradadır. İş ise diğer worktree\'lerde yapılır. <strong>Geçerli yap</strong> bunlardan birini varsayılan worktree\'ye getirir: commit\'leri, henüz commit etmediği değişiklikleri ve yeni dosyaları, varsayılan worktree\'nin dalının üzerine. O andan itibaren başlık her ekranda varsayılan worktree\'nin ne çalıştırdığını söyler — <code>VARSAYILAN · feat-a + feat-b · 1 düzenleme</code> — ve kenar çubuğu <em>varsayılan</em> ve <em>geçerli</em> worktree\'leri işaretler.',
    '<img src="{img}current.png" alt="Başlıktaki şerit, açık: varsayılan worktree\'nin nereden geldiği, iki geçerli worktree ve sonra yapılabilecekler" loading="lazy">',
    '<strong>Değiştir</strong> yerine başka bir worktree koyar. <strong>Ekle</strong> iki değişikliği birlikte denemek için bir tane daha üstüne ekler; ikisi aynı satırlara dokunduğunda yükleme varsayılan worktree\'de bir çakışmada durur ve her dosya için varsayılan tarafı korur, geleni alır ya da düzenleyip çözüldü olarak işaretlersin — sonra <em>Devam et</em> ya da önceki duruma dönmek için <em>İptal et</em>.',
    '<strong>Test sırasında yapılan düzeltmeler eve döner.</strong> Denerken sen — ya da varsayılan worktree\'deki bir ajan — bir şeyi düzeltirsen, <em>Düzenlemeleri geri gönder</em> onu ait olduğu worktree\'ye, orada gözden geçirip commit etmen için commit edilmemiş değişiklikler olarak geri verir; <em>Geri al</em> bir sonraki adıma kadar bunu geri alır. Birden çok geçerli worktree\'ye ya da hiçbirine ait olmayan bir dosya tahmin edilmez, sorulur. Değiştir, Ekle, Yenile ve Bitir önce düzenlemeleri geri gönderir; böylece varsayılan worktree\'de yapılan hiçbir şey kaybolmaz.',
    '<strong>Bitir</strong> varsayılan worktree\'yi tam olarak eskisi gibi geri verir: dalını ve hazırlanmış ya da hazırlanmamış kendi değişikliklerini. Her adım önce kısa bir önizleme açar: ne geliyor, ne kenara alınıyor, hangi dosyalar çakışır ve şu anda neyin engel olduğu, çıkış yoluyla birlikte. Bunları bir worktree\'nin <code>···</code> menüsünde, kenar çubuğunda sağ tıkla, başlık şeridinde ve ⌘⇧D / ⌘⇧E ile bulursun.',
    'Ayarlar › Git, ajanı çalışmaya devam ettiğinde geçerli bir worktree\'yi kendiliğinden yenileyebilir ve her yüklemeden sonra bir komutu — örneğin <code>pnpm-lock.yaml</code> değiştiğinde <code>pnpm install</code> — varsayılan worktree\'nin terminalinde, görebileceğin yerde çalıştırabilir. Kendi <code>git stash</code>\'ine asla dokunulmaz. git 2.40 veya daha yenisi gerekir.',
  ]),
  ...
  ('trouble', 'Bir şeyler ters gittiğinde', [
    ...
    '<p><strong>Uyarıdan tamamen kaçınmak.</strong> Terminal’den kur: <code>curl -fsSL https://mhguitarte.github.io/ygd-editor/install.sh | bash</code>. İndirilen dosyayı macOS’un yapacağı gibi denetler ve uygulamayı Uygulamalar klasörüne koyar; indirme sayfasına bak.</p>'
