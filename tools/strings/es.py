...
T = {
 ...
 'cli_h': 'Sin el aviso: instálala desde Terminal.',
 'cli_p': 'Pega esto en Terminal (Aplicaciones › Utilidades). Descarga la última versión para tu Mac, comprueba su checksum y que está firmada con el certificado del proyecto, y la deja en Aplicaciones. macOS no pregunta nada, porque nada ha pasado por un navegador. Vuelve a ejecutarlo cuando quieras para reinstalar.',
 'cli_copy': 'Copiar', 'cli_copied': 'Copiado',
 'cli_or': 'O, con la imagen de disco del botón de arriba:',
 'need_h': 'Qué necesitas', 'need_p': 'Git en tu ordenador y una cuenta en al menos un proveedor. <a href="https://github.com/openai/codex">Codex</a> y <a href="https://github.com/google-gemini/gemini-cli">Gemini CLI</a> no van dentro del instalador —por eso ocupa poco—: la aplicación descarga el que elijas la primera vez que lo pidas (unos 100 MB para Codex, 20 MB para Gemini CLI) y lo comprueba contra la versión con la que se hizo esta release antes de ejecutarlo; para <a href="https://docs.anthropic.com/en/docs/claude-code">Claude Code</a>, cuya licencia no permite incluirlo, Ajustes › Proveedores de IA ofrece un clic que ejecuta por ti el instalador oficial de Anthropic. Conectar un proveedor abre su página de inicio de sesión en tu navegador. Para los pull requests, <code>gh</code> de GitHub o <code>glab</code> de GitLab.',
 ...
 'g_sections': [
  ...
  ('current', 'Probar un worktree en el worktree por defecto', [
    'El sitio para probar un cambio es la carpeta del propio repositorio —el <em>worktree por defecto</em>—, porque ahí están ya <code>node_modules</code>, <code>.env</code>, la caché de compilación y el servidor de desarrollo. El trabajo se hace en los otros worktrees. <strong>Hacer actual</strong> trae uno de ellos al worktree por defecto: sus commits, lo que aún no ha commiteado y sus archivos nuevos, encima de la rama del worktree por defecto. Desde entonces la cabecera dice, en todas las pantallas, qué ejecuta el worktree por defecto —<code>POR DEFECTO · feat-a + feat-b · 1 edición</code>— y la barra lateral marca el worktree <em>por defecto</em> y los <em>actuales</em>.',
    '<img src="{img}current.png" alt="La franja de la cabecera, abierta: de dónde vino el worktree por defecto, los dos worktrees actuales y qué se puede hacer después" loading="lazy">',
    '<strong>Cambiar</strong> pone otro worktree en su lugar. <strong>Añadir</strong> apila otro encima, para probar dos cambios juntos; donde ambos tocan las mismas líneas la carga se detiene por un conflicto en el worktree por defecto, y para cada archivo mantienes el lado por defecto, tomas el entrante o lo editas y lo marcas resuelto; luego <em>Continuar</em>, o <em>Abortar</em> para volver a lo que se ejecutaba antes.',
    '<strong>Los arreglos hechos al probar vuelven a casa.</strong> Si tú —o un agente en el worktree por defecto— arreglas algo mientras pruebas, <em>Devolver ediciones</em> lo devuelve al worktree al que pertenece, como cambios sin commitear para revisar y commitear allí; <em>Deshacer</em> lo revierte hasta el siguiente paso. Un archivo que pertenece a varios worktrees actuales, o a ninguno, se pregunta en vez de adivinarse. Cambiar, Añadir, Actualizar y Terminar devuelven primero las ediciones, así que nada de lo hecho en el worktree por defecto se pierde.',
    '<strong>Terminar</strong> devuelve el worktree por defecto exactamente como estaba: su rama y sus propios cambios, preparados y sin preparar. Cada paso abre antes una vista previa breve: qué entra, qué se guarda aparte, qué archivos entrarían en conflicto y lo que lo impide ahora mismo, con la salida. Están en el menú <code>···</code> de un worktree, con clic derecho en la barra lateral, en la franja de la cabecera y en ⌘⇧D / ⌘⇧E.',
    'Ajustes › Git puede actualizar solo un worktree actual cuando su agente sigue trabajando, y ejecutar un comando tras cada carga —<code>pnpm install</code> cuando cambió <code>pnpm-lock.yaml</code>, por ejemplo— en el terminal del worktree por defecto, donde lo ves. Tu <code>git stash</code> nunca se toca. Necesita git 2.40 o posterior.',
  ]),
  ...
  ('trouble', 'Cuando algo falla', [
    ...
    '<p><strong>Evitar el aviso por completo.</strong> Instálala desde Terminal: <code>curl -fsSL https://mhguitarte.github.io/ygd-editor/install.sh | bash</code>. Comprueba la descarga como lo habría hecho macOS y deja la aplicación en Aplicaciones; más en la página de descarga.</p>'
    ...
    '<p><strong>Un proveedor aparece como «No instalado».</strong> Codex y Gemini CLI se descargan a petición: pulsa <em>Descargar</em> en la tarjeta y, si la descarga falla, la tarjeta dice por qué (sin conexión, o un archivo que no coincidía con su suma de comprobación y se descartó). Para Claude Code, pulsa <em>Instalar Claude Code</em> en su tarjeta; si lo instalaste tú, abre una terminal, comprueba que <code>claude --version</code> funciona y luego Ajustes › Proveedores de IA › Volver a buscar.</p>'
  ...
  ('uninstall', 'Desinstalar', [
    '<strong>Antes de quitarla</strong> —las dos cosas son opcionales—. Si hay un worktree <em>actual</em> en el worktree por defecto, pulsa <em>Terminar</em>, para que el worktree por defecto recupere su rama y sus cambios. Para cerrar sesión en un proveedor, usa <em>Desconectar</em> en su tarjeta de Ajustes › Proveedores de IA: las sesiones son de Claude Code, Codex y Gemini CLI, no de la aplicación, así que quitar la aplicación no las cierra. Después cierra ygd-editor.',
    '<strong>macOS.</strong> Arrastra <strong>ygd-editor</strong> de Aplicaciones a la Papelera —desde <code>~/Applications</code> si la instalación por Terminal la puso ahí.',
    '<strong>Windows.</strong> Configuración › Aplicaciones › Aplicaciones instaladas › ygd-editor › <em>Desinstalar</em>, o Panel de control › Programas y características.',
    '<strong>Linux.</strong> Borra el archivo AppImage. Para el paquete de Debian / Ubuntu: <code>sudo apt remove ygd-editor</code>.',
    '<strong>Lo que se queda.</strong> Tus ajustes, espacios de trabajo, historial de sesiones y las descargas de Codex y Gemini CLI están en la carpeta <code>.ygd-editor</code> de tu directorio personal, y desinstalar no los borra, así que al reinstalar sigues donde lo dejaste. Para quitarlos también, junto con las cachés de la aplicación, ejecuta esto después de desinstalar:',
    ...
    '<strong>Lo que nunca se toca.</strong> Tus repositorios y los worktrees que creaste son carpetas de git normales y siguen donde están; <code>git worktree remove &lt;carpeta&gt;</code> quita uno. Si la aplicación instaló Claude Code por ti, sigue en tu directorio personal: la <a href="https://docs.anthropic.com/en/docs/claude-code">documentación de Anthropic</a> explica cómo quitarlo.',
  ]),
