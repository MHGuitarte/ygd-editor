...
T = {
 'en': {
  ...
  'g_sections': [
   ...
   ('daily', 'Day to day', [
     '<p><strong>The main worktree</strong> — the folder you opened the repository at — is listed with the others and is the one new worktrees branch from. Select it and <em>Switch branch</em> in the header puts it on any branch: yours, or one that only exists on the remote, which it checks out as a new tracking branch. A branch another worktree already holds is listed but greyed out, because git allows a branch in one worktree at a time.</p>'
   ...
   ('trouble', 'When something goes wrong', [
     ...
     '<p><strong>A provider shows “Not installed”.</strong> Codex and Gemini CLI come with the app, so that means the app’s copy is damaged: reinstall the app. For Claude Code, click <em>Install Claude Code</em> on its card; if you installed it yourself, open a terminal, check that <code>claude --version</code> works, then Settings › AI providers › Rescan.</p>'
 ...
 'es': {
  ...
  'g_sections': [
   ...
   ('daily', 'En el día a día', [
     '<p><strong>El worktree principal</strong> —la carpeta con la que abriste el repositorio— aparece junto a los demás y es aquel del que se ramifican los nuevos. Selecciónalo y <em>Cambiar de rama</em> en la cabecera lo pone en cualquier rama: una tuya o una que solo exista en el remoto, que se crea como rama de seguimiento. Una rama que ya tenga otro worktree aparece en gris, porque git solo permite una rama en un worktree a la vez.</p>'
